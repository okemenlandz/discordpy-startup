...
async def m_symphogear(ctx, rate_str=None):
	...
	in_money = math.ceil(normal_cnt / 9.91) * unit_cost
...
async def m_gen(ctx, rate_str=None):
	...
	unit_cost = round(125 * rate)
	in_money = math.ceil(normal_cnt / 10.46) * unit_cost
...
async def m_gen2(ctx, rate_str=None):
	...
	unit_cost = round(125 * rate)
	in_money = math.ceil(normal_cnt / 8.65) * unit_cost
...
async def m_aria(ctx, rate_str=None):
	...
	unit_cost = round(125 * rate)
	in_money = math.ceil(normal_total / 8.43) * unit_cost
...
async def m_goyoku(ctx, rate_str=None):
	...
	unit_cost = round(125 * rate)
	in_money = math.ceil(normal_cnt / 9.25) * unit_cost
...
async def m_madoka3(ctx, rate_str=None):
	...
	unit_cost = round(125 * rate)
	in_money = math.ceil(normal_cnt / 9.86) * unit_cost
...
@bot.command()
async def takt(ctx):
	# 通常時: 1/464.79 運命チャージ(300個)、1/1394.38 で1500個
	normal_cnt = 0
	normal_total = 0
	total_net = 0
	total_disp = 0
	...
	while True:
		v = random.randint(0, 65535)
		normal_cnt += 1
		if v < 47:          # 1/1394.38 → 1500個
			normal_total += normal_cnt
			break
		elif v < 188:       # 1/464.79 → 運命チャージ(300個)
			await ctx.send(f'[{ctx.author}] {normal_cnt}G 運命チャージ')
			total_net += 280
			total_disp += 300
			normal_total += normal_cnt
			normal_cnt = 0
	...
	if random.randint(0, 1) == 0:
		...
		await ctx.send(f'[{ctx.author}] Destiny Challenge 突入')
		dc_detail = []
		for _ in range(5):
...
async def m_takt(ctx, rate_str=None):
	if not await _check_m_permission(ctx):
		return
	if rate_str is None:
		await ctx.send('レートを入力してください（例: /m-takt 1）')
		return
	try:
		rate = _parse_rate(rate_str)
	except ValueError:
		await ctx.send('レートは0より大きく1以下の数を指定してください（例: /m-takt 1）')
		return
